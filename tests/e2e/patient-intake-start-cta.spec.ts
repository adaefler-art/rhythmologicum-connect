...
test.describe('patient start intake CTA @patient-start-intake', () => {
  ...
  test('opens fastpass form-first page from start entry CTA', async ({ page }) => {
    test.skip(backendMode !== 'mock', 'Test only runs in mock backend mode.')
    ...
    await expect(page.getByRole('button', { name: 'Fastpass starten' }).first()).toBeVisible()
    await page.getByRole('button', { name: 'Fastpass starten' }).first().click()

    await expect(page).toHaveURL(/\/patient\/fastpass/)
    await expect(page.getByText('Wartezimmer-Fastpass').first()).toBeVisible()
    await expect(page.getByRole('button', { name: 'Fastpass abschliessen' }).first()).toBeVisible()
  })
})

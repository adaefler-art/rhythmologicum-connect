/**
 * E75.2: Patient Anamnesis API - Create Version
 * 
 * POST /api/patient/anamnesis/[entryId]/versions
 * 
 * Creates a new version of an existing anamnesis entry.
 * Automatically increments version number via database trigger.
 * 
 * Security:
 * - Requires authentication
 * - RLS ensures patient can only update their own entries
 * - Returns 404 if entry not found or not accessible
 * - Returns 409 if entry is archived (optional constraint)
 * 
 * Request Body:
 * {
 *   title: string,
...
export async function POST(request: Request, context: RouteContext) {
  try {
    ...
    // Check if entry is archived (optional 409 policy)
    ...
    if (updateError) {
      console.error('[patient/anamnesis/versions POST] Update error:', updateError)
      return NextResponse.json(
        {
          success: false,
          error: {
    ...
    if (versionError) {
      console.error('[patient/anamnesis/versions POST] Version fetch error:', versionError)
    ...
    console.error('[patient/anamnesis/versions POST] Unexpected error:', err)
    return NextResponse.json(
      {
        success: false,
        error: {

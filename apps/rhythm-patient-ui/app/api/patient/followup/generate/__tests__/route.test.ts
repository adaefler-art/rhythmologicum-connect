...
describe('POST /api/patient/followup/generate', () => {
  ...
  it('does not ask medication gap again after partial medication question answered with nein.', async () => {
    const queryBuilder = {
      _mode: 'select' as 'select' | 'update',
      select: jest.fn(function select() {
        this._mode = 'select'
  ...
  it('does not ask medication gap again after partial medication question answered with nei', async () => {
    const queryBuilder = {
      _mode: 'select' as 'select' | 'update',
      select: jest.fn(function select() {
        this._mode = 'select'
  ...
  it('maps clinician-request onset answer into structured data and avoids onset loop', async () => {
            ...
            data: {
              ...
              structured_data: {
                ...
                followup: {
                  next_questions: [
                    {
                      id: 'clinician-request:seit-wann-bestehen-die-beschwerden',
                      question: 'Seit wann bestehen die Beschwerden?',
                      why: 'Rueckfrage aus aerztlicher Pruefung',
                      priority: 1,
                      source: 'clinician_request',
    ...
    const request = new Request('http://localhost/api/patient/followup/generate', {
      ...
      body: JSON.stringify({
        intakeId: '11111111-1111-4111-8111-111111111111',
        asked_question_id: 'clinician-request:seit-wann-bestehen-die-beschwerden',
        asked_question_text: 'Seit wann bestehen die Beschwerden?',
        asked_answer_text: 'Seit drei Tagen',
      }),
    })
    ...
    const nextQuestionIds = (json?.data?.next_questions ?? []).map((entry: { id?: string }) =>
      String(entry?.id ?? ''),
    )
    expect(nextQuestionIds).not.toContain('gap:onset')

    const persistedOnset = (
    ...
    expect(persistedOnset).toBe('Seit drei Tagen')
  })
})

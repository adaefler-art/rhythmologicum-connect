...
ensure_npm() {
	...
	if [[ ! -x "${NODE_HOME}/bin/npm" ]]; then
		echo "[ci_pre_xcodebuild][app-local] npm not found; installing Node ${NODE_VERSION} (${NODE_ARCH})"
...
if ! command -v npm >/dev/null 2>&1; then
	echo "[ci_pre_xcodebuild][app-local] ERROR: npm still not found after bootstrap. PATH=${PATH}"
	exit 1
fi

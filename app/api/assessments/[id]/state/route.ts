 ...
 *
 * Returns the current state of an assessment including:
 * - Assessment status
 * - Current step index
 ...
 *     stepIndex: number,
 *     answersByQuestionId: { [questionId: string]: number | string | boolean }
 *   }
 * }
 */
  ...
  try {
    ...
    if (assessment.patient_id !== patientProfile.id) {
      ...
      return forbiddenResponse(
        'Sie haben keine Berechtigung, dieses Assessment zu sehen.',
        correlationId,
      )
    }

    // Load assessment state using persistence adapter
    const assessmentRun = await loadAssessmentRun(assessmentId)

    return versionedSuccessResponse(assessmentRun, PATIENT_ASSESSMENT_SCHEMA_VERSION, 200, correlationId)
  } catch (error) {
